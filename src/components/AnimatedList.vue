<template>
  <div :class="cn('flex flex-col items-center gap-4', $props.class)">
    <transition-group name="list" tag="div" class="flex flex-col-reverse items-center gap-3 w-full" move-class="move">
      <!-- Only render the items up to the current index -->
      <Motion v-for="data in itemsToShow" :key="data.id" as="div" :initial="{ scale: 0, opacity: 0 }" :animate="{
        scale: 1,
        opacity: 1,
        y: 0,
      }" :exit="{
...
async function startLoop() {
  const notificationsRaw = slots.default ? slots.default()[0].children ?? [] : [];
