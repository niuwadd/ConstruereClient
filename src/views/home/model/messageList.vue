    ...
    <template v-for="(item, index) in agentMessageList" :key="index">
      ...
      <div
        ...
        <div v-if="item.messageType === MessageType.IMAGE" class="w-[calc(100%-30px)] max-w-[300px]">
          <img class="w-full" :src="typeof item.text === 'string' ? item.text : ''">
        </div>
        ...
        <!-- 当数据是数组 -->
        <div v-else-if="Array.isArray(item.text)" class="grid grid-cols-2 gap-4 lg:grid-cols-3 xl:grid-cols-4">
        ...
        <!-- 当数据是mackdown -->
        <div v-else-if="item.messageType === MessageType.MARKDOWN">
          <MdPreview class="px-2 bg-[#f8f8f6]!" v-model="item.text" />
        </div>
        <!-- <div v-else-if="Array.isArray(item.text)" class="flex flex-col gap-4">
          <div v-for="product in item.text" :key="product.productId"
            class="flex gap-4 rounded-2xl shadow-[0_0_10px_4px_rgba(0,0,0,0.1)] p-2">
            <img class="w-[100px] rounded-2xl" :src="product.logo">
            <div class="flex flex-col gap-4">
              <div class="flex justify-between">
                <h3 class="text-lg">{{ product.productName }}</h3>
                <p class="text-xs">{{ product.productPrice }}</p>
              </div>
              <p class="text-xs">{{ product.productContent }}</p>
            </div>
          </div>
        </div> -->
